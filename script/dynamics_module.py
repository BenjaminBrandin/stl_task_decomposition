...
class ImpactSolverLP:
    ...
    s.t A u <= b
    """
    def __init__(self, agent:Agent, max_velocity:List[float]) -> None:
        ...
        self.cost  = self.Lg.T @ agent.symbolic_input # change of sign becaise you have to turn maximization into minimization
        self.theta = ca.MX.sym('theta', 1)

        self.param = ca.vertcat(self.Lg,self.theta)

        A_func, b_func = create_rectangular_constraint_function([[-max_velocity[0], max_velocity[0]], [-max_velocity[1], max_velocity[1]]])
        A = A_func(self.theta)
        b = b_func(self.theta)
        constraints = A@agent.symbolic_input - b # this is already an ca.MX that is a funciton of the control input

        
        with NoStdStreams():  
            lp          = {'x':agent.symbolic_input, 'f':self.cost, 'g':constraints, 'p':self.param} # again we make it parameteric to avoid rebuilding the optimization program since the structure is always the same
            self.solver      = ca.qpsol('S', 'qpoases', lp,{"printLevel":"none"}) # create a solver object 
    
    def maximize(self,Lg:np.ndarray, theta) -> np.ndarray:
        ...
        param = np.concatenate((-Lg,theta.reshape(1, 1)), axis=1)
        return self.solver(p=param,ubg=0)["x"]
        # return self.solver(p=-Lg,ubg=0)["x"]
    
    def minimize(self,Lg:np.ndarray, theta) -> np.ndarray:
        """This program will simply be solved as a function of the parameter. This avoids re bulding the optimization program every time"""
